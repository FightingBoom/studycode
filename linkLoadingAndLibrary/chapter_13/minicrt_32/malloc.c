...
{
    enum {
        HEAP_BLOCK_FREE = 0xABABABAB,   // magic of free block
...
{
    heap_header* header
        = (heap_header*)ADDR_ADD(ptr, -HEADER_SIZE);
    if (header->type != HEAP_BLOCK_USED)
        return;

    header->type = HEAP_BLOCK_FREE;
    if (header->prev != NULL
        && header->prev->type == HEAP_BLOCK_FREE)
    {
        // merge
    ...
    if (header->next != NULL
        && header->next->type == HEAP_BLOCK_FREE)
    {
        // merge
...
void* malloc(unsigned size)
{
    ...
    while (header != 0)
    {
        ...
        if (header->size > size + HEADER_SIZE * 2)
        {
            // split
            heap_header* next
                = (heap_header*)ADDR_ADD(header, size + HEADER_SIZE);
            next->prev = header;
